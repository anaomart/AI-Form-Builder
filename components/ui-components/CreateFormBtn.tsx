...
export default function CreateFormBtn() {
  ...
  async function onSubmit(values: formSchemeType) {
    ...
    if (usingAI) {
      ...
      const questions = await  GenerateQuestions(values.description)
        let parsed = Array.from(JSON.parse(questions)) as FormElementInstance[]
        console.log("parsed");
        console.log({parsed});
        console.log("parsed");
      setQuestionsResponse(parsed);
    }
    try {
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-[400px] rounded-xl">
        <DialogHeader>
          ...
          <DialogDescription>Create a New Form with AI</DialogDescription>
        </DialogHeader>
        <Form {...form}>
