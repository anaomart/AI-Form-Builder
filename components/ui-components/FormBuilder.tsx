...
export default function FormBuilder({ form }: { form: Form }) {
  const { setElements, setSelectedElement, elements, addElement } =
    useDesigner();
  const [suggestedQuestionState, setSuggestedQuestionState] = useState<
  ...
  const DesignerElement =
    FormElements[suggestionsQuestion[0]?.type || "TextField"].designerComponent;
        ...
        <AnimatePresence>
          {suggestionsQuestion?.length > 0 && (
            <motion.div
              ...
              exit={{
                y: suggestedQuestionState == "removed" ? -200 : 1000,
                opacity: 0.2,
              }}
